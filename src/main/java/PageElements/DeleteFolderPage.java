...
public class DeleteFolderPage extends Driver
{
	...
	public boolean Deletefolder(String Foldername) throws Exception
	{
		...
		{
			//((JavascriptExecutor)driver).executeScript("arguments[0].style.border='3px solid red'", driver.findElement(By.xpath(Xpathbeforetablerow + (i+1) + Xpathaftertablerow)));
			if(driver.findElement(By.xpath(Xpathbeforetablerow + (i+1) + Xpathaftertablerow )).getText().contains(Foldername))
			{
				
				//((JavascriptExecutor)driver).executeScript("arguments[0].style.border='4px solid red'", driver.findElement(By.xpath(Xpathbeforetablerow + (i+1) + Xpathaftertablecol)));
